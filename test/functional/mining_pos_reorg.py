...
class ReorgStakeTest(infincoincashTestFramework):
    ...
    def check_money_supply(self, expected_icc):
        # verify that nodes have the expected ICC supply
        icc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(icc_supply, [DecimalAmt(expected_icc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that ICC supply was properly updated after the reorgs
        self.log.info("Check ICC supply...")
