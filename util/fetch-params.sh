...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/INFINCOINCASHParams"
    else
        PARAMS_DIR="$HOME/.infincoincash-params"
    fi
fi
...
Failed to fetch the INFINCOINCASH zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
INFINCOINCASH - fetch-params.sh

This script will fetch the INFINCOINCASH zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common INFINCOINCASH zkSNARK parameters. Note that it is
