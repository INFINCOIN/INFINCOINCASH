...
#if defined(NDEBUG)
#error "INFINCOINCASH cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uicc) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("infincoincash-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> iccInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                iccInputs.push_back(stakeIn);
        }
        const bool hasICCInputs = !iccInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasICCInputs) {
                    ...
                    for (const CTxIn& iccIn : iccInputs)
                        if(iccIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasICCInputs) {
                            for (const CTxIn& stakeIn : iccInputs)
