...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("INFINCOINCASHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("INFINCOINCASHMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("INFINCOINCASHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("infincoincash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running INFINCOINCASHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("INFINCOINCASHMiner exception");
    } catch (...) {
        LogPrintf("INFINCOINCASHMiner exception");
    }

    LogPrintf("INFINCOINCASHMiner exiting\n");
}
