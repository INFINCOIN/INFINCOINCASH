...
CIccStake* CIccStake::NewIccStake(const CTxIn& txin)
{
    ...
    return new CIccStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CIccStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CIccStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CIccStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CIccStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CIccStake::GetUniqueness() const
{
    //The unique identifier for a ICC stake is the outpoint
...
const CBlockIndex* CIccStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CIccStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CIccStake::ContextCheck(int nHeight, uint32_t nTime)
{
