...
const char * const INFINCOINCASH_CONF_FILENAME = "infincoincash.conf";
const char * const INFINCOINCASH_PID_FILENAME = "infincoincash.pid";
const char * const INFINCOINCASH_MASTERNODE_CONF_FILENAME = "masternode.conf";


// INFINCOINCASH only features
// Masternode
...
#else
    const char* pszModule = "infincoincash";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\INFINCOINCASH
// Windows >= Vista: C:\Users\Username\AppData\Roaming\INFINCOINCASH
// Mac: ~/Library/Application Support/INFINCOINCASH
// Unix: ~/.infincoincash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "INFINCOINCASH";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\INFINCOINCASHParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\INFINCOINCASHParams
    // Mac: ~/Library/Application Support/INFINCOINCASHParams
    // Unix: ~/.infincoincash-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "INFINCOINCASHParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "INFINCOINCASHParams";
#else
    // Unix
    return pathRet / ".infincoincash-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/infincoincash/sapling-spend.params";
        sapling_output = "/usr/share/infincoincash/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/infincoincash/sapling-spend.params";
            sapling_output = "/usr/local/share/infincoincash/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", INFINCOINCASH_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", INFINCOINCASH_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty infincoincash.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override infincoincash.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", INFINCOINCASH_PID_FILENAME));
