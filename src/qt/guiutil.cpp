...
#define URI_SCHEME "infincoincash"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter INFINCOINCASH address (e.g. %1)").arg("iRbpRBdmahOJ2RsVzoAzyFNbFqcgbX45CU"));
...
{
    QRegularExpression rx("^(\\d{0,12})((\\.|,)\\d{1,6})?$");
...
{
    // return if URI is not valid or is no INFINCOINCASH: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::ICC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert infincoincash:// to infincoincash:
    //
    //    Cannot handle this later, because infincoincash:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ICC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "INFINCOINCASH (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "INFINCOINCASH (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "INFINCOINCASH.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for INFINCOINCASH*.lnk
...
{
    return GetAutostartDir() / "infincoincash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a infincoincash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=INFINCOINCASH (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=INFINCOINCASH (regtest)\n";
        else
            optionFile << "Name=INFINCOINCASH\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the infincoincash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add infincoincash app to startup item list
