...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select ICC Outputs to Spend" :
            "Select Shielded ICC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::ICC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show INFINCOINCASH address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
