...
#include "qt/infincoincash/send.h"
#include "qt/infincoincash/forms/ui_send.h"
#include "qt/infincoincash/addnewcontactdialog.h"
#include "qt/infincoincash/qtutils.h"
#include "qt/infincoincash/sendchangeaddressdialog.h"
#include "qt/infincoincash/optionbutton.h"
#include "qt/infincoincash/sendconfirmdialog.h"
#include "qt/infincoincash/guitransactionsutils.h"
#include "qt/infincoincash/loadingdialog.h"
...
SendWidget::SendWidget(INFINCOINCASHGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-icc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onICCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onICCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent ICCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield ICCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent ICCs to shield."));
    }
}
...
void SendWidget::onICCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto iccAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(iccAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(iccAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(iccAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
