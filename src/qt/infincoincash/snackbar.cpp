...
#include "qt/infincoincash/forms/ui_snackbar.h"
#include "qt/infincoincash/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(INFINCOINCASHGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &INFINCOINCASHGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
