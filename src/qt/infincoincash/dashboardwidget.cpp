...
#include "qt/infincoincash/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/infincoincash/forms/ui_dashboardwidget.h"
#include "qt/infincoincash/qtutils.h"
#include "qt/infincoincash/sendconfirmdialog.h"
#include "qt/infincoincash/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(INFINCOINCASHGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareIcc, "square-chart-icc");
    setCssProperty(ui->labelIcc, "text-chart-icc");
    ...
    setCssProperty(ui->labelAmountIcc, "text-stake-icc-disable");
    ...
    if (window)
        connect(window, &INFINCOINCASHGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("ICC")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ICC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            icc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalIcc += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesIcc.append(icc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(241, 179, 47));
    ...
    set0->append(chartData->valuesIcc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalIcc > 0) {
        setCssProperty(ui->labelAmountIcc, "text-stake-icc");
    } else {
        setCssProperty(ui->labelAmountIcc, "text-stake-icc-disable");
    }
    forceUpdateStyle({ui->labelAmountIcc});
    ui->labelAmountIcc->setText(GUIUtil::formatBalance(chartData->totalIcc, nDisplayUnit));
