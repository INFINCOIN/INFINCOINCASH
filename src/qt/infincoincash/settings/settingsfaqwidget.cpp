...
#include "qt/infincoincash/settings/settingsfaqwidget.h"
#include "qt/infincoincash/qtutils.h"
#include "qt/infincoincash/settings/forms/ui_settingsfaqwidget.h"
#include <QMetaObject>
#include <QScrollBar>

SettingsFaqWidget::SettingsFaqWidget(INFINCOINCASHGUI* parent) : QDialog(parent),
                                                                           ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({ui->labelSubtitle_Intro,
                       ui->labelSubtitle_UnspendableICC,
    ...
    setCssProperty({ui->labelContent_Intro,
                       ui->labelContent_UnspendableICC,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("INFINCOIN CASH is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. INFINCOIN CASH incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("INFINCOIN CASH utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of INFINCOIN CASH to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableICCContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received INFINCOIN CASH requires 6 confirmations on the network "
               "to become eligible for spending which can take ~12-15 minutes.")) +
        formatFAQParagraph(
            tr("Your INFINCOIN CASH wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableICC->setText(unspendableICCContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of INFINCOIN CASH with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("infincoin cash", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("For Support you can contact us on our Email: support@infincoin.com")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node INFINCOIN CASH wallet with a "
               "requirement of 1,000,000,000 ICC secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in INFINCOIN CASH Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the INFINCOIN CASH network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("1,000,000,000 ICC per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 1,000,000,000 ICC collateral "
    ...
    connect(ui->pushButton_UnspendableICC, &QPushButton::clicked, [this]() { onFaqClicked(ui->widget_UnspendableICC); });
    ...
    if (parent)
        connect(parent, &INFINCOINCASHGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
