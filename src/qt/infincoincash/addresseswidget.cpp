...
#include "qt/infincoincash/forms/ui_addresseswidget.h"
#include "qt/infincoincash/addresslabelrow.h"
#include "qt/infincoincash/addnewaddressdialog.h"
#include "qt/infincoincash/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(INFINCOINCASHGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto iccAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(iccAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(iccAdd);
        if (walletModel->updateAddressBookLabels(iccAdd, label.toUtf8().constData(),
