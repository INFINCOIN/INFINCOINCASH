...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ICC);
    ...
    if (settings.contains("nAnonymizeinfincoincashAmount") || reset)
        gArgs.SoftSetArg("-anonymizeinfincoincashamount", settings.value("nAnonymizeinfincoincashAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in infincoincash.cpp during shotdown to also remove the window positions
