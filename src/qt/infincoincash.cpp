...
#include "config/infincoincash-config.h"
#endif
...
#include "qt/infincoincash/splash.h"
#include "qt/infincoincash/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("infincoin cash", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in infincoincash.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in infincoincash.qrc)
...
/** Class encapsulating INFINCOIN CASH startup and shutdown.
...
/** Main INFINCOINCASH application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (INFINCOINCASHGUI)
...
private:
    ...
    INFINCOINCASHGUI* window{nullptr};
...
{
    window = new INFINCOINCASHGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &INFINCOINCASHGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &INFINCOINCASHGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(INFINCOINCASHGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(INFINCOINCASHGUI::DEFAULT_WALLET);
        ...
        // INFINCOINCASH: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &INFINCOINCASHGUI::handlePaymentRequest);
        connect(window, &INFINCOINCASHGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. INFINCOIN CASH can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(infincoincash_locale);
    Q_INIT_RESOURCE(infincoincash);
    ...
    /// 6. Determine availability of data directory and parse infincoincash.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("INFINCOIN CASH"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("INFINCOIN CASH"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // infincoincash: links repeatedly have their payment requests routed to this process:
